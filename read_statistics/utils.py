...
def read_statistics_once_read(request, obj):
    ...
    if not request.COOKIES.get(key):
        # 总阅读数 +1
        readnum, created = ReadNum.objects\
            .get_or_create(content_type=ct, object_id=obj.pk)
        readnum.read_num += 1
        readnum.save()

        # 当天阅读数 +1
        date = timezone.now().date()
        readDetail, created = ReadDetail.objects\
            .get_or_create(content_type=ct, object_id=obj.pk, date=date)
